...
void inline SinCos( float radians, float *sine, float *cosine )
{
...
#elif defined( PLATFORM_WINDOWS_PC32 ) && defined( _MSC_VER ) // r47: _asm is MSVC-only
	_asm
	{
	...
	*sine = sin( radians );
	*cosine = cos( radians );
#elif defined( POSIX ) || defined( __GNUC__ ) // r47: mingw uses the fsincos path too
...
FORCEINLINE unsigned long RoundFloatToUnsignedLong(float f)
{
	...
	#if defined( _WIN32 ) && defined( _MSC_VER ) // r47: _asm is MSVC-only
		__asm
		{
			fld f
			fistp       qword ptr nResult
		}
	#elif defined( __MINGW32__ ) // r47
		__asm__ __volatile__ ( "fistpll %0;": "=m" (nResult) : "t" (f) : "st" );
	#elif POSIX
		__asm __volatile__ (
