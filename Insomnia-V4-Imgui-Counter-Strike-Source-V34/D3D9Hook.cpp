...
{
	// config loads only via menu button, so until first load MenuKey is 0 -> fall back to INSERT
	int menuKey = g_CVars.Miscellaneous.MenuKey;
	if( menuKey <= 0 ) menuKey = VK_INSERT;
	if( uMsg == WM_KEYDOWN && wParam == ( WPARAM )menuKey )
	{
		bMouse = !bMouse;
		printconsole( "[Awesware] menu %s (key 0x%X)\n", bMouse ? "OPEN" : "CLOSED", menuKey );
	...
	if( g_bImGuiInitialized && bMouse )
	{
		...
		{
			if( uMsg >= WM_KEYFIRST && uMsg <= WM_KEYLAST && ( int )wParam != menuKey )
				return 1;
		}
	}
...
HRESULT __stdcall Hooked_EndScene( IDirect3DDevice9* pDevice )
{
	...
	if( !g_bImGuiInitialized )
	{
		...
		if( g_hGameWindow )
		{
			...
			g_GUI.SetupStyle( );
			GUI_BuildIconsV3( ); // r46: rip the V3 icons into the atlas
