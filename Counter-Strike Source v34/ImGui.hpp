...
{
	// Main
	IMGUI_API bool          ColorPicker(float col[3]);
	...
	void AddTriangleFilledMultiColor(const ImVec2 & a, const ImVec2 & b, const ImVec2 & c, ImU32 col_a, ImU32 col_b, ImU32 col_c);
	...
	IMGUI_API bool			KeyButton(const char* label, int* key, const ImVec2& size = ImVec2(0, 0));
	...
	IMGUI_API bool			ColorPicker(const char * label, ImColor * color, float col[3]);
	IMGUI_API bool			ColorPicker(const char * label, ImColor * color);
	...
	IMGUI_API bool Checkbox1(const char * label, bool * v);
