...
WeaponInfo NoSpread::GetWeaponInfo( CSWeapon* Weapon )
{
	...
	// mingw port: identical to the former MSVC asm block
	// (pushes range-ptr, power-ptr, bullet-type; callee cleans the stack = stdcall).
	typedef void ( __stdcall *GetBulletTypeParameters_t )( int, float*, float* );
	( ( GetBulletTypeParameters_t )0x241CE4B0 )( TempWeaponInfo.BulletType, &TempWeaponInfo.PenetrationPower, &TempWeaponInfo.PenetrationRange );
