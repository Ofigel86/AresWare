...
void CVars::Init( )
{
	...
	Visuals.ASUS = 1.f;
	Visuals.Indicators = true; // on-screen FL/DT/AA indicators
	Visuals.PlayerList = false; // r44: separate player-list window (V3 menu)
	Miscellaneous.SlowWalk = true; // r41: slow walk ON out of the box
	Miscellaneous.SlowWalkKey = 6; // r41: SHIFT (ComboKeyVK index 6)
	Miscellaneous.SlowWalkSpeed = 120;
	Miscellaneous.AutoStrafeMode = 0; // r38: classic by default
	Miscellaneous.StrafeAvoidDist = 64; // r38: wall probe distance
	Aimbot.ForceMinDmgValue = 10;
	Aimbot.RageGroups = false; // r45: global settings by default
	for( int rgi = 0; rgi < 5; rgi++ )
	{
		Aimbot.RageGroup[ rgi ].AutoShoot = true; Aimbot.RageGroup[ rgi ].AutoStop = false;
		Aimbot.RageGroup[ rgi ].AutoWall = true; Aimbot.RageGroup[ rgi ].MultiSpot = true;
		Aimbot.RageGroup[ rgi ].HitScan = true; Aimbot.RageGroup[ rgi ].BodyVsJump = false;
		Aimbot.RageGroup[ rgi ].BodyAWP = false; Aimbot.RageGroup[ rgi ].AntiSMAC = false;
		Aimbot.RageGroup[ rgi ].HitChance = false; Aimbot.RageGroup[ rgi ].StrictPrimary = false;
		Aimbot.RageGroup[ rgi ].BestDamage = true;
		Aimbot.RageGroup[ rgi ].MinDamage = 0; Aimbot.RageGroup[ rgi ].HitChanceValue = 0;
		Aimbot.RageGroup[ rgi ].Hitbox = 12; Aimbot.RageGroup[ rgi ].FallbackHitbox = 0;
	}
	...
	// menu + exploit defaults (applied at inject, Load overrides from file)
	Miscellaneous.MenuKey = VK_INSERT;
	Miscellaneous.MenuTheme = 0;
	Miscellaneous.DoubleTap = false;
	Miscellaneous.Fakeduck = false; // r24
	g_CVars.Aimbot.FallbackHitbox = 0; // r25: 0 = legacy group scan
	g_CVars.Aimbot.Interpolation.DisableInterp = false; // r25
	g_CVars.Visuals.SpectatorList = false; // r25
	Miscellaneous.FakeduckKey = 0; // r24
	Miscellaneous.Micromoves = false; // r24
	Miscellaneous.ClientModEmulator = true; // r34: anti-detect on by default
	Miscellaneous.DoubleTapAuto = true;
	Miscellaneous.DoubleTapTicks = 2;
	Miscellaneous.DoubleTapMode = 0;
	Miscellaneous.ThirdPersonKey = 0;
	Miscellaneous.ThirdPersonDist = 120;
	Aimbot.BacktrackTicks = 6;
	Aimbot.AimFOV = 90;
	Aimbot.LongRangeDist = 1000;
	Miscellaneous.MenuMode = 1; // r13: Aimware V4 look by default (switch back in CONFIGS)
	Miscellaneous.AntiAim.FlickRandom = true; // anti-metronome flicks by default
	Miscellaneous.AntiAim.FlickOnShot = false;

	// legitbot defaults (own settings; rage base inherited for the rest)
	Legit.Active = false;
	Legit.AutoShoot = false;
	Legit.Silent = false;
	Legit.SnapLimiter = true;
	Legit.AngleLimit = 8;
	Legit.AngleLimitTens = 0.f;
	Legit.Key = 0;
	Legit.AimFOV = 4;
	Legit.Hitbox = 0;
	Legit.AutoStop = false;
	Legit.BacktrackTicks = 6;
	Legit.AimType = 2; // adaptive smooth
	Legit.Smoothing = 8;
	Legit.ReactionMs = 120;
	Legit.RCS = 70;
	Legit.RCSStandalone = true;
	Legit.AimLock = true;
	Legit.DesyncResolver = true;
	Legit.DesyncAA = true; // r15 legit AA: fake (0 deg) on choked ticks, real view untouched
	Legit.DesyncYaw = 0;
	Legit.DesyncChoke = 6; // r43: auto-fakelag depth while the legit AA fake is armed
	Legit.FovNear = 14; // r29: wide cone for close enemies
	Legit.FovFar = 5; // r29: tight cone for far enemies
	Legit.FovSwitchDist = 450; // r29: distance where near cone becomes far
	Legit.FlashCheck = true;
	Legit.LegitAA = false;
	Legit.LegitAAKey = 0; // 0 = always while enabled
	Legit.LegitAAInvertKey = 0; // hold: flip pad side
	Legit.LegitAAAngle = 14;
	Legit.TargetSelection = 4; // crosshair
	Legit.StrafeActive = false;
	Legit.StrafePower = 4;
	for( int lg = 0; lg < 7; lg++ ) Legit.HitboxGroup[ lg ] = 1;
	Triggerbot.Delay = 90;
	Legit.Prediction = true;
	Legit.FovCircle = true;
	Legit.AutoPistol = true;
	Legit.KillDelayMs = 250;
}
...
void Stuff::ForceSeed( CUserCmd* pCmd, BasePlayer* Target )
{
	// r12: on this engine the bullet seed is DERIVED FROM command_number
	// (MD5_PseudoRandom), so we roll command_number until its low byte is the
	// one producing the MINIMAL spread offset for the CURRENT weapon. Replaces
	// the hardcoded seed 141 (which was tuned for nothing in particular).
	float flSpread = 0.f;
	BasePlayer* fsLocal = ( BasePlayer* ) g_pClientEntityList->GetClientEntity( g_pEngineClient->GetLocalPlayer( ) );
	if( fsLocal )
	{
		CSWeapon* fsWeapon = ( CSWeapon* ) fsLocal->GetActiveBaseCombatWeapon( );
		if( fsWeapon && fsWeapon->IsWeapon( ) ) flSpread = fsWeapon->GetSpread( );
	}
	...
	// r23: basis + trace for the hit-aware pass (Target != 0)
	Vector fwd, right, up;
	AngleVectors( pCmd->viewangles, &fwd, &right, &up );
	TraceFilterSkipTwoEntities fsFilter( fsLocal, 0 );
	Vector fsEye = ( fsLocal ) ? fsLocal->EyePosition( ) : Vector( 0, 0, 0 );
	...
	for( int low = 0; low < 256; low++ )
	{
		...
		if( mag < bestMag ) { bestMag = mag; bestLow = low; }

		if( Target && fsLocal )
		{
			Vector dir = fwd + sx * right + sy * up;
			VectorNormalizeFast( dir );
			Vector end = fsEye + dir * 8192.f;
			Ray_t ray; ray.Init( fsEye, end );
			trace_t tr;
			g_pEngineTrace->TraceRay( ray, 0x46004003, ( ITraceFilter* )&fsFilter, &tr );
			if( tr.m_pEnt == Target && mag < hitMag ) { hitMag = mag; hitLow = low; }
		}
	}
	if( hitLow >= 0 ) bestLow = hitLow; // r23: a seed that lands on the target beats a merely tiny spread

	for( ;; ++pCmd->command_number )
	{
		pCmd->random_seed = MD5_PseudoRandom( pCmd->command_number ) & 0x7FFFFFFF;
		if( ( pCmd->random_seed & 0xFF ) == bestLow ) break;
	}
}
...
void Stuff::NoRecoil( CUserCmd* pCmd, BasePlayer* LocalPlayer, bool isAA )
{
	...
	// r10: SDK-exact punch decay (cs_gamemovement.cpp CCSGameMovement::DecayPunchAngle):
	//   len -= (10 + len*0.5) * frametime, each tick, clamped to 0.
	// A shot released from a fakelag/DT backlog is processed by the server AFTER
	// one decay per queued tick, so the raw current punch OVER-corrects every
	// choked shot. Compensate with the punch decayed by the backlog instead.
	QAngle punch = LocalPlayer->GetPunchAngle( );
	{
		int backlog = g_iChokedTicks;
		if( backlog > 16 ) backlog = 16;
		for( int d = 0; d < backlog; d++ )
		{
			float px = punch.x, py = punch.y, pz = punch.z;
			float len = sqrtf( px * px + py * py + pz * pz );
			if( len < 0.0001f ) { punch = QAngle( 0, 0, 0 ); break; }
			float inv = 1.f / len;
			px *= inv; py *= inv; pz *= inv;
			len -= ( 10.f + len * 0.5f ) * g_pGlobals->interval_per_tick;
			if( len < 0.f ) len = 0.f;
			punch = QAngle( px * len, py * len, pz * len );
		}
	}
	...
	if( g_CVars.Miscellaneous.AntiAim.Active )
	{
		if( isAA )
		{
			pCmd->viewangles.x = ( 180 - pCmd->viewangles.x ) - ( punch.x * 2 );
			pCmd->viewangles.y = -( ( 180 - pCmd->viewangles.y ) + ( punch.y * 2 ) );
		}
		else pCmd->viewangles -= punch * 2;
	}
	else pCmd->viewangles -= punch * 2;
}
...
// ---- r38: slophook directional autostrafe (semantic port) ----
// Faithful to slophook_reconstructed: avoidWalls + directional (90/100 step),
// key precedence back > left > right, tie in wall turn goes to PLUS.
static float SlophNorm( float x ) { while( x < -180.f ) x += 360.f; while( x > 180.f ) x -= 360.f; return x; }

static float Sloph_VelocityYaw( BasePlayer* p )
{
	Vector v = p->m_vecVelocity( );
	return atan2f( v.y, v.x ) * 57.2957763671875f;
}

// Source hull trace (NOT a line ray): Ray_t carries the extents.
static float Sloph_HullTrace( BasePlayer* lp, const Vector& origin, const Vector& end, const Vector& mins, const Vector& maxs, Vector& normalOut )
{
	Ray_t ray; ray.Init( origin, end, mins, maxs );
	trace_t tr;
	TraceFilterSkipTwoEntities filter( lp, 0 );
	g_pEngineTrace->TraceRay( ray, 0x0200400b, ( ITraceFilter* )&filter, &tr );
	normalOut = tr.plane.normal;
	return tr.fraction;
}

// Returns true only when a forward wall is detected.
static bool Sloph_AvoidWalls( BasePlayer* lp, float wantedYaw, float distance, float ipt, float& outYaw )
{
	Vector vel = lp->m_vecVelocity( );
	float speed = sqrtf( vel.x * vel.x + vel.y * vel.y );
	float height = ( lp->m_fFlags( ) & FL_DUCKING ) ? 45.f : 72.f;
	if( speed > 40.f )
	{
		float vYaw = Sloph_VelocityYaw( lp );
		wantedYaw = SlophNorm( vYaw + SlophNorm( wantedYaw - vYaw ) * 0.5f ); // blend halfway to velocity dir
	}
	if( speed > 0.f )
	{
		float probe = speed * ipt * 4.f;
		distance = ( probe > distance ) ? probe : distance;
	}

	Vector origin = lp->m_vecOrigin( );
	Vector mins( -16.f, -16.f, 0.f ), maxs( 16.f, 16.f, height );
	Vector n;
	float a = wantedYaw * 0.01745329238474369f;
	Vector end( origin.x + cosf( a ) * distance, origin.y + sinf( a ) * distance, origin.z );
	float frontFrac = Sloph_HullTrace( lp, origin, end, mins, maxs, n );
	if( frontFrac >= 1.f || fabs( n.z ) >= 0.7f ) return false; // clear ahead / floor-or-ceiling plane
	float pa = ( wantedYaw + 40.f ) * 0.01745329238474369f;
	Vector pend( origin.x + cosf( pa ) * distance, origin.y + sinf( pa ) * distance, origin.z );
	float plusFrac = Sloph_HullTrace( lp, origin, pend, mins, maxs, n );
	float ma = ( wantedYaw - 40.f ) * 0.01745329238474369f;
	Vector mend( origin.x + cosf( ma ) * distance, origin.y + sinf( ma ) * distance, origin.z );
	float minusFrac = Sloph_HullTrace( lp, origin, mend, mins, maxs, n );
	float turn = ( 1.f - frontFrac ) * 80.f + 20.f;
	outYaw = SlophNorm( wantedYaw + ( ( plusFrac >= minusFrac ) ? turn : -turn ) ); // tie goes to PLUS
	return true;
}

void Stuff::AutoStrafeDirectional( CUserCmd* pCmd, BasePlayer* LocalPlayer )
{
	Vector vel = LocalPlayer->m_vecVelocity( );
	float speed = sqrtf( vel.x * vel.x + vel.y * vel.y );
	float vYaw = Sloph_VelocityYaw( LocalPlayer );
	if( speed < 5.f ) vYaw = pCmd->viewangles.y;
	const bool forward = ( pCmd->buttons & IN_FORWARD ) != 0;
	const bool back = ( pCmd->buttons & IN_BACK ) != 0;
	const bool left = ( pCmd->buttons & IN_MOVELEFT ) != 0;
	const bool right = ( pCmd->buttons & IN_MOVERIGHT ) != 0;
	float offset = 0.f;
	// Original precedence: back > left > right. Contradictory keys are NOT
	// reduced to a vector; preserving this tree matters for W+S / A+D.
	if( back )
	{
		if( left ) offset = 135.f;
		else offset = right ? 225.f : 180.f;
	}
	else if( left ) offset = forward ? 45.f : 90.f;
	else if( right ) offset = forward ? -45.f : -90.f;
	float wanted = SlophNorm( pCmd->viewangles.y + offset );
	float adjusted;
	if( Sloph_AvoidWalls( LocalPlayer, wanted, ( float )g_CVars.Miscellaneous.StrafeAvoidDist, g_pGlobals->interval_per_tick, adjusted ) )
		wanted = SlophNorm( adjusted );
	float delta = SlophNorm( wanted - vYaw );
	if( speed >= 5.f )
	{
		// Deliberate 90/100 degree heuristic, NOT asin(idealSpeed/speed).
		float step = ( fabs( delta ) > 100.f ) ? 100.f : 90.f;
		if( delta < 0.f ) step = -step; // delta == 0 selects +90
		wanted = SlophNorm( vYaw + step );
	}
	float angle = ( pCmd->viewangles.y - wanted ) * 0.01745329238474369f;
	pCmd->forwardmove = cosf( angle ) * 400.f;
	pCmd->sidemove = sinf( angle ) * 400.f;
}
...
void Stuff::AutoStrafe( CUserCmd* pCmd, BasePlayer* LocalPlayer )
{
	...
	// r38: slophook dispatcher preconditions for directional (air, no ladder/noclip);
	// classic keeps the hold-space path.
	if( g_CVars.Miscellaneous.AutoStrafeMode == 1 )
	{
		if( LocalPlayer->m_MoveType( ) == 9 || LocalPlayer->m_MoveType( ) == 8 ) return;
		if( LocalPlayer->m_fFlags( ) & FL_ONGROUND ) return;
		AutoStrafeDirectional( pCmd, LocalPlayer );
		return;
	}
...
CTickRecord pPlayerHistory[ 65 ][ 32 ];
CTickRecord pSimulationData[ 65 ];
CTickRecord pBackupData[ 65 ];
