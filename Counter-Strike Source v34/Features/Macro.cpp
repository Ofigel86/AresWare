...
void Macro::SaveMacro( CRecord* movement, Vector *DrawPath, int &TickEnd, const char* filename )
{
	...
	if( fp != NULL )
	{
		//Snapshot the length: the legacy code decremented the caller's
		//TickEnd through fwrite() arguments, corrupting playback state.
		int count = TickEnd;
		if( count < 0 ) count = 0;
		if( count >= MaxRecordTicks ) count = MaxRecordTicks - 1;

		fwrite( &count, sizeof( int ), 1, fp );
		fwrite( DrawPath, sizeof( Vector ), count, fp );
		fwrite( movement, sizeof( CRecord ), ( count > 0 ) ? count - 1 : 0, fp );
		fclose( fp );
	}
	else
...
void Macro::ReadMacro( CRecord* movement, Vector *DrawPath, int &TickEnd, const char* filename )
{
	...
	if( fp != NULL )
	{
		...
		//Reject corrupt/oversized headers instead of fread()ing with a
		//wild count (negative counts become huge size_t values).
		if( TickEnd < 1 || TickEnd >= MaxRecordTicks )
		{
			TickEnd = 0;
			fclose( fp );
			return;
		...
		fread( movement, sizeof( CRecord ), TickEnd - 1, fp );
		TickEnd -= 2; // legacy format semantics: playback uses indices 0..TickEnd inclusive
		fclose( fp );
	}
	else
...
void Macro::ReadMacro2( SRecord* movement, int &tick_count, Vector &StartPosition, const char* filename )
{
	...
	if( fp != NULL )
	{
		...
		if( tick_count < 1 || tick_count >= MaxRecordTicks )
		{
			tick_count = 0;
			fclose( fp );
			return;
