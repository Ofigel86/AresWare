...
static DormantCache_t s_dorm[ 65 ];

void OutOfFOVArrows( )
{
	if( !g_CVars.Visuals.ESP.OOF ) return;
	BasePlayer* LocalPlayer = ( BasePlayer* )g_pClientEntityList->GetClientEntity( g_pEngineClient->GetLocalPlayer( ) );
	if( !LocalPlayer || LocalPlayer->m_lifeState( ) != 0 ) return;
	QAngle va; g_pEngineClient->GetViewAngles( va );
	float yawRad = va.y * 0.01745329252f;
	float fx = cos( yawRad ), fy = sin( yawRad ); // view forward on ground plane
	float rx = fy, ry = -fx; // view right
	Vector eye = LocalPlayer->EyePosition( );
	float cx = screen_x * 0.5f, cy = screen_y * 0.5f;
	float radius = screen_y * 0.30f;
	for( int i = 1; i <= g_pGlobals->maxClients; i++ )
	{
		if( i == g_pEngineClient->GetLocalPlayer( ) ) continue;
		BasePlayer* Ent = ( BasePlayer* )g_pClientEntityList->GetClientEntity( i );
		if( !Ent ) continue;
		bool dorm = Ent->IsDormant( );
		if( dorm && ( !g_CVars.Visuals.ESP.Dormant || !s_dorm[ i ].valid ) ) continue;
		if( !dorm && Ent->m_lifeState( ) != 0 ) continue;
		int team = dorm ? s_dorm[ i ].team : Ent->m_iTeamNum( );
		if( g_CVars.Visuals.ESP.EnemyOnly && team == LocalPlayer->m_iTeamNum( ) ) continue;
		if( g_CVars.PlayerList.Friend[ i ] ) continue;
		Vector mid = ( dorm ? s_dorm[ i ].origin : Ent->GetAbsOrigin( ) ) + Vector( 0, 0, 36 );
		Vector test = mid + Vector( 0, 0, 36 );
		Vector sp;
		if( g_Stuff.WorldToScreen( test, sp ) && sp.x > 0 && sp.x < screen_x && sp.y > 0 && sp.y < screen_y ) continue;
		Vector d = mid - eye;
		float dist2d = sqrt( d.x * d.x + d.y * d.y );
		if( dist2d < 1.f ) continue;
		float tx = d.x / dist2d, ty = d.y / dist2d;
		float ang = atan2( tx * rx + ty * ry, tx * fx + ty * fy ); // 0 = ahead, + = right
		float dx = sin( ang ), dy = -cos( ang );
		float px = cx + dx * radius, py = cy + dy * radius;
		float margin = 24.f;
		if( px < margin ) px = margin; if( px > screen_x - margin ) px = screen_x - margin;
		if( py < margin ) py = margin; if( py > screen_y - margin ) py = screen_y - margin;
		Color col = dorm ? Color( 130, 130, 130, 220 ) : ( ( team == 2 ) ? Color( 255, 90, 90, 220 ) : Color( 120, 170, 255, 220 ) );
		float sz = 10.f;
		float tipx = px + dx * sz, tipy = py + dy * sz;
		float perpx = -dy, perpy = dx;
		float bx = px - dx * sz * 0.7f, by = py - dy * sz * 0.7f;
		float b1x = bx + perpx * sz * 0.7f, b1y = by + perpy * sz * 0.7f;
		float b2x = bx - perpx * sz * 0.7f, b2y = by - perpy * sz * 0.7f;
		g_Drawing.Line( ( int )tipx, ( int )tipy, ( int )b1x, ( int )b1y, col );
		g_Drawing.Line( ( int )tipx, ( int )tipy, ( int )b2x, ( int )b2y, col );
		g_Drawing.Line( ( int )b1x, ( int )b1y, ( int )b2x, ( int )b2y, col );
		char dtxt[ 16 ]; sprintf( dtxt, "%dm", ( int )( dist2d / 50.f ) );
		g_Drawing.MenuStringNormal( true, false, ( int )px, ( int )py + 10, Color( 255, 255, 255, 160 ), "%s", dtxt );
	}
}

// LOCAL ANIM FIX (stage 1): firstperson desync compass, drawn below the crosshair.
// Shows where REAL (white) and FAKE (cyan) point relative to your view; the split
// between the two lines is the live desync. Up on the compass = looking direction.
// Same toggle as FakeAngleViz (Visuals ESP Fake), no new CVars.
// on-screen feature indicators: fakelag choke count, doubletap charge, AA desync
void Indicators( )
{
	if( !g_CVars.Visuals.Indicators ) return;
	BasePlayer* LocalPlayer = ( BasePlayer* ) g_pClientEntityList->GetClientEntity( g_pEngineClient->GetLocalPlayer( ) );
	if( !LocalPlayer || LocalPlayer->m_lifeState( ) != 0 ) return;

	// watermark: name + fps (top-left)
	static float fpsSmooth = 60.f;
	float ft = g_pGlobals->frametime;
	if( ft > 0.0001f && ft < 1.f ) fpsSmooth = fpsSmooth * 0.95f + ( 1.f / ft ) * 0.05f;
	g_Drawing.MenuStringNormal( false, false, 12, 8, Color( 255, 255, 255, 220 ), "awesware | %d fps", ( int )( fpsSmooth + 0.5f ) );

	// left-side stack
	int ix = 26;
	int iy = screen_y / 2 - 46;

	// FL: choked ticks (green -> red as the choke grows)
	int choke = g_iChokedTicks;
	if( choke < 0 ) choke = 0; if( choke > 99 ) choke = 99;
	int cr = choke * 20; if( cr > 255 ) cr = 255;
	int cg = 255 - choke * 14; if( cg < 60 ) cg = 60;
	g_Drawing.MenuStringNormal( false, false, ix, iy, Color( cr, cg, 80, 230 ), "FL  %d", choke );
	iy += 20;

	// DT: charge bar + state (only when doubletap is on)
	if( g_CVars.Miscellaneous.DoubleTap )
	{
		int need = g_CVars.Miscellaneous.DoubleTapTicks + 2;
		if( need < 1 ) need = 1;
		bool ready = ( g_DTCharge >= need );
		float frac = ( float )g_DTCharge / ( float )need;
		if( frac < 0.f ) frac = 0.f; if( frac > 1.f ) frac = 1.f;
		Color dtCol = ready ? Color( 120, 255, 120, 230 ) : Color( 255, 220, 100, 230 );
		if( ready ) g_Drawing.MenuStringNormal( false, false, ix, iy, dtCol, "DT  READY" );
		else
		{
			char bar[ 12 ]; int fill = ( int )( frac * 10.f );
			for( int b = 0; b < 10; b++ ) bar[ b ] = ( b < fill ) ? '=' : '-';
			bar[ 10 ] = 0;
			g_Drawing.MenuStringNormal( false, false, ix, iy, dtCol, "DT  [%s] %d%%", bar, ( int )( frac * 100.f ) );
		}
		iy += 20;
	}

	// AA: fake/real desync in degrees (only when antiaim is on)
	if( g_CVars.Miscellaneous.AntiAim.Active )
	{
		float desync = g_Stuff.GuwopNormalize( g_flAAFakeYaw - g_flAARealYaw );
		if( desync < 0.f ) desync = -desync;
		g_Drawing.MenuStringNormal( false, false, ix, iy, Color( 120, 220, 255, 230 ), "AA  %.0f", desync );
	}
}

void DesyncArrow( )
{
	if( !g_CVars.Visuals.ESP.Fake ) return;
	if( !g_CVars.Miscellaneous.AntiAim.Active ) return;
	BasePlayer* LocalPlayer = ( BasePlayer* )g_pClientEntityList->GetClientEntity( g_pEngineClient->GetLocalPlayer( ) );
	if( !LocalPlayer || LocalPlayer->m_lifeState( ) != 0 ) return;
	float desync = g_Stuff.GuwopNormalize( g_flAAFakeYaw - g_flAARealYaw );
	if( desync < 0.f ) desync = -desync;
	if( desync < 5.f ) return; // no meaningful desync
	QAngle view; g_pEngineClient->GetViewAngles( view );
	float cx = ( float )( screen_x / 2 ), cy = ( float )( screen_y / 2 + 90 ), r = 40.f;
	// yaw+ = turn left on screen (screen-left = world-up x view-dir), hence x = cx - r*sin(d)
	float dReal = g_Stuff.GuwopNormalize( g_flAARealYaw - view.y ) * 0.0174533f;
	float dFake = g_Stuff.GuwopNormalize( g_flAAFakeYaw - view.y ) * 0.0174533f;
	g_Drawing.Line( ( int )cx, ( int )cy, ( int )( cx - r * sin( dReal ) ), ( int )( cy - r * cos( dReal ) ), Color( 255, 255, 255, 220 ) );
	g_Drawing.Line( ( int )cx, ( int )cy, ( int )( cx - r * sin( dFake ) ), ( int )( cy - r * cos( dFake ) ), Color( 0, 255, 255, 220 ) );
	g_Drawing.MenuStringNormal( true, false, ( int )cx, ( int )( cy + r + 4 ), Color( 0, 255, 255, 220 ), "%.0f", desync );
}
...
void FakeAngleViz( )
{
	...
	if( desync < 0.f ) desync = -desync;
	if( desync < 5.f ) return; // no meaningful desync
	g_Drawing.DrawBones( LocalPlayer, Color( 255, 255, 255, 130 ) ); // real (white)
	g_Drawing.DrawBonesRotated( LocalPlayer, Color( 0, 255, 255, 220 ), g_flAAFakeYaw - g_flAARealYaw ); // fake (cyan)
	Vector head = LocalPlayer->GetAbsOrigin( ) + Vector( 0, 0, 78 );
	Vector hs;
	if( g_Stuff.WorldToScreen( head, hs ) )
		g_Drawing.MenuStringNormal( true, false, ( int )hs.x, ( int )hs.y - 10, Color( 0, 255, 255, 220 ), "%.0f", desync );
}
...
void BoundingBoxESP( )
{
	...
	for( int Index = 1; Index <= g_pGlobals->maxClients; Index++ )
	{
		...
		if( isDormant && ( !g_CVars.Visuals.ESP.Dormant || !s_dorm[ Index ].valid ) ) continue;
		if( !isDormant && !( Ent->m_lifeState( ) == 0 ) ) { s_dorm[ Index ].valid = false; continue; }
		int entTeam = isDormant ? s_dorm[ Index ].team : Ent->m_iTeamNum( );
		if( g_CVars.Visuals.ESP.EnemyOnly && entTeam == localTeam ) continue;
		...
		{
			if( entTeam == 2 ) colour = g_CVars.ColorSelector.ESP.TT; //Color::Red( );
			else if( entTeam == 3 ) colour = g_CVars.ColorSelector.ESP.CT; //Color::LightBlue( );
		}

		if( isDormant ) colour = Color( 130, 130, 130, 160 ); // dormant: faded grey

		if( !isDormant )
		{
			s_dorm[ Index ].valid = true;
			s_dorm[ Index ].origin = Ent->GetAbsOrigin( );
			s_dorm[ Index ].duck = ( Ent->m_fFlags( ) & FL_DUCKING ) != 0;
			s_dorm[ Index ].team = Ent->m_iTeamNum( );
			s_dorm[ Index ].hp = Ent->m_iHealth( );
			s_dorm[ Index ].seenTime = g_pGlobals->curtime;
		}
		vPlayerFoot = isDormant ? s_dorm[ Index ].origin : Ent->GetAbsOrigin( );
		bool bDucking = isDormant ? s_dorm[ Index ].duck : ( ( Ent->m_fFlags( ) & FL_DUCKING ) != 0 );
		...
		if( g_CVars.Visuals.ESP.Bone && !isDormant ) g_Drawing.DrawBones( Ent, Color( 255, 255, 255, 160 ) );
		...
		if( g_CVars.Visuals.ESP.Box )
		{
			Color boxCol( colour.r( ), colour.g( ), colour.b( ), 180 );
			Color boxOut( 0, 0, 0, 140 );
			int bstyle = g_CVars.Visuals.ESP.BoxStyle;
			if( bstyle == 1 ) g_Drawing.CornerBox( ( int )box.x, ( int )box.y, ( int )( HalfWidth * 2 ), ( int )Height, boxCol, boxOut );
			else if( bstyle == 2 )
			{
				Vector org = Ent->GetAbsOrigin( );
				float top = bDucking ? 53.5f : 72.f;
				Vector bp[ 9 ];
				bp[ 0 ] = org + Vector( -18, -18, 0 ); bp[ 1 ] = org + Vector( 18, -18, 0 );
				bp[ 2 ] = org + Vector( 18, 18, 0 ); bp[ 3 ] = org + Vector( -18, 18, 0 );
				bp[ 4 ] = bp[ 2 ] + Vector( 0, 0, top ); bp[ 5 ] = bp[ 1 ] + Vector( 0, 0, top );
				bp[ 6 ] = bp[ 0 ] + Vector( 0, 0, top ); bp[ 7 ] = bp[ 3 ] + Vector( 0, 0, top );
				bp[ 8 ] = org + Vector( 0, 0, top );
				g_Drawing.Box3D( bp, boxCol );
			}
			else g_Drawing.OutlinedBox( box.x, box.y, ( HalfWidth * 2 ), Height, boxCol, boxOut );
		}

		if( g_CVars.Visuals.ESP.AimSpot && !isDormant ) g_Drawing.DrawAimSpot( Ent, g_CVars.Aimbot.Hitbox, Color( 255, 255, 255, 160 ) );

		g_pEngineClient->GetPlayerInfo( Index, &PlayerInfo );

		if( g_CVars.Visuals.ESP.Name )
		{
			g_Drawing.MenuStringNormal( true, false, box.x + HalfWidth, box.y - 13 - ( ( g_CVars.Visuals.ESP.HealthStyle == 2 && ( g_CVars.Visuals.ESP.Health || g_CVars.Visuals.ESP.Armor ) ) ? 13.f : 0.f ), Color( 255, 255, 255, 200 ),
		...
		float bottomPad = 0.f;
		if( Health > 0 && !isDormant )
		{
			if( Health > 100 ) Health = 100;
			int Scale = ( int )( Health * 2.55f );
			if( Scale < 0 ) Scale = 0; if( Scale > 255 ) Scale = 255;
			Color hpCol( 255 - Scale, Scale, 0, 200 );
			Color patch( 0, 0, 0, 128 );
			if( g_CVars.Visuals.ESP.Health )
			{
				float frac = Health / 100.f;
				float bw = HalfWidth * 2.f;
				int hstyle = g_CVars.Visuals.ESP.HealthStyle;
				if( hstyle == 1 ) // left vertical + number
				{
					float bx = box.x - 6.f;
					g_Drawing.OutlinedRect( ( int )bx - 1, ( int )box.y - 1, 6, ( int )Height + 2, patch );
					g_Drawing.FilledRect( ( int )bx, ( int )box.y, 4, ( int )Height, patch );
					int fh = ( int )( Height * frac );
					g_Drawing.FilledRect( ( int )bx, ( int )( box.y + Height - fh ), 4, fh, hpCol );
					g_Drawing.MenuStringNormal( true, false, ( int )bx + 2, ( int )box.y - 13, Color( 255, 255, 255, 200 ), "%d", Ent->m_iHealth( ) );
				}
				else if( hstyle == 2 ) // top + number
				{
					g_Drawing.OutlinedRect( ( int )box.x - 1, ( int )box.y - 7, ( int )bw + 2, 4, patch );
					g_Drawing.FilledRect( ( int )box.x, ( int )box.y - 6, ( int )bw, 2, patch );
					g_Drawing.FilledRect( ( int )box.x, ( int )box.y - 6, ( int )( bw * frac ), 2, hpCol );
					g_Drawing.MenuStringNormal( true, false, ( int )( box.x + bw ) + 12, ( int )box.y - 8, Color( 255, 255, 255, 200 ), "%d", Ent->m_iHealth( ) );
				}
				else // bottom (classic)
				{
					float by = box.y + Height + 3.f + bottomPad;
					g_Drawing.OutlinedRect( ( int )box.x - 1, ( int )by - 1, ( int )bw + 2, 4, patch );
					g_Drawing.FilledRect( ( int )box.x, ( int )by, ( int )bw, 2, patch );
					g_Drawing.FilledRect( ( int )box.x, ( int )by, ( int )( bw * frac ), 2, hpCol );
					bottomPad += 5.f;
				}
			}
		}
		// armor bar (blue)
		if( g_CVars.Visuals.ESP.Armor && !isDormant )
		{
			int Armor = Ent->m_ArmorValue( );
			if( Armor > 100 ) Armor = 100;
			if( Armor > 0 )
			{
				float afrac = Armor / 100.f;
				float bw = HalfWidth * 2.f;
				Color apCol( 90, 150, 255, 200 );
				Color apatch( 0, 0, 0, 128 );
				int hstyle = g_CVars.Visuals.ESP.HealthStyle;
				if( hstyle == 1 ) // right vertical
				{
					float ax = box.x + bw + 2.f;
					g_Drawing.OutlinedRect( ( int )ax - 1, ( int )box.y - 1, 6, ( int )Height + 2, apatch );
					g_Drawing.FilledRect( ( int )ax, ( int )box.y, 4, ( int )Height, apatch );
					int fah = ( int )( Height * afrac );
					g_Drawing.FilledRect( ( int )ax, ( int )( box.y + Height - fah ), 4, fah, apCol );
				}
				else if( hstyle == 2 ) // above top bar
				{
					g_Drawing.OutlinedRect( ( int )box.x - 1, ( int )box.y - 12, ( int )bw + 2, 4, apatch );
					g_Drawing.FilledRect( ( int )box.x, ( int )box.y - 11, ( int )bw, 2, apatch );
					g_Drawing.FilledRect( ( int )box.x, ( int )box.y - 11, ( int )( bw * afrac ), 2, apCol );
				}
				else // under bottom bar
				{
					float ay = box.y + Height + 3.f + bottomPad;
					g_Drawing.OutlinedRect( ( int )box.x - 1, ( int )ay - 1, ( int )bw + 2, 4, apatch );
					g_Drawing.FilledRect( ( int )box.x, ( int )ay, ( int )bw, 2, apatch );
					g_Drawing.FilledRect( ( int )box.x, ( int )ay, ( int )( bw * afrac ), 2, apCol );
					bottomPad += 4.f;
				}
			}
		}
		...
		if( Weapon && !isDormant )
		{
			...
			if( g_CVars.Visuals.ESP.Ammo )
			{
				char ab[ 16 ]; sprintf( ab, " [%d]", Weapon->m_iClip1( ) );
				strcat( weaponName, ab );
			}
		...
		if( isDormant )
		{
			float since = g_pGlobals->curtime - s_dorm[ Index ].seenTime;
			g_Drawing.MenuStringNormal( true, false, box.x + HalfWidth, box.y + Height + 3, Color( 130, 130, 130, 200 ), "DORMANT %.0f", since );
		}

		Vector clientorigin = pBackupData[ Index ].m_Origin;
		Vector serverorigin = pPlayerHistory[ Index ][ 0 ].m_Origin;

		if( !isDormant && g_CVars.Aimbot.Interpolation.LagPrediction > 0 && clientorigin != Vector( 0, 0, 0 ) && serverorigin != Vector( 0, 0, 0 ) )
		{
			Vector w2s[ 2 ];
...
void GroundESP( )
{
	...
	for( int i = g_pClientEntityList->GetMaxEntities( ); i >= 0; --i )
	{
		...
		// FPS: cheap reject before the expensive 8x transform + 8x WorldToScreen below.
		// Only dropped weapons/grenades/C4 are ever drawn, so filter by class first.
		const char* className = Ent->GetClientClass( )->m_pNetworkName;
		if( !className || className[ 0 ] != 'C' ) continue;
		if( g_pClientEntityList->GetClientEntityFromHandle( Ent->m_hOwnerEntity( ) ) ) continue; // carried, not dropped
		char c2 = className[ 1 ];
		bool maybeWeapon = ( c2 == 'W' || c2 == 'A' || c2 == 'D' || c2 == 'C' || c2 == 'P' || c2 == 'S' || c2 == 'F' || c2 == 'H' );
		if( !maybeWeapon ) continue;
		...
		std::string name = Ent->GetClientClass( )->m_pNetworkName; // only weapon-ish classes reach this point
...
// r25: spectator list - dead players whose observer camera targets the local player
static void DrawSpectatorList( void )
{
	static int offTarget = 0, offMode = 0;
	if( !offTarget )
	{
		offTarget = HackInterfaces::NetvarManager( ).GetOffset( "DT_BasePlayer", "m_hObserverTarget" );
		offMode = HackInterfaces::NetvarManager( ).GetOffset( "DT_BasePlayer", "m_iObserverMode" );
	}
	if( offTarget <= 0 || offMode <= 0 ) return; // netvars unavailable - no list

	int localIdx = g_pEngineClient->GetLocalPlayer( );
	BasePlayer* LocalPlayer = ( BasePlayer* )g_pClientEntityList->GetClientEntity( localIdx );
	if( !LocalPlayer ) return;

	int x = screen_x - 200;
	int y = 90;
	int shown = 0;
	for( int i = 1; i <= g_pGlobals->maxClients && shown < 10; i++ )
	{
		if( i == localIdx ) continue;
		BasePlayer* e = ( BasePlayer* )g_pClientEntityList->GetClientEntity( i );
		if( !e || e->IsDormant( ) ) continue;
		if( e->m_lifeState( ) == 0 ) continue; // alive players do not spectate
		int mode = *( int* )( ( DWORD_PTR )e + offMode );
		if( mode != 4 && mode != 5 ) continue; // 4=fixed 5=in-eye: they literally watch us
		DWORD h = *( DWORD* )( ( DWORD_PTR )e + offTarget );
		if( ( int )( h & 0xFFF ) != localIdx ) continue; // EHANDLE: low bits = entity index
		player_info_t info;
		if( !g_pEngineClient->GetPlayerInfo( i, &info ) ) continue;
		g_Drawing.MenuStringNormal( true, false, x, y + shown * 14, Color( 255, 130, 130, 255 ), "[spec] %s", info.name );
		shown++;
	}
}
...
void __fastcall Hooked_PaintTraverse( void* ptr, int edx, unsigned int vguiPanel, bool forceRepaint, bool allowForce )
{
	...
	if( bValid )
	{
		...
		if( g_pEngineClient->IsInGame( ) )
		{
			...
			HitESP( );
			if( g_CVars.Visuals.EventLog ) g_Drawing.DrawEventLog( );
			if( g_CVars.Visuals.SpectatorList ) DrawSpectatorList( ); // r25
			if( g_CVars.Visuals.ESP.Ground ) GroundESP( );
			BoundingBoxESP( );
			FakeAngleViz( );
			DesyncArrow( );
			Indicators( );
			LuaAPI::Paint( ); // lua on_paint
			OutOfFOVArrows( );
			Crosshair( );
			if( g_CVars.Legit.Active && !g_CVars.Aimbot.Active && g_CVars.Legit.FovCircle )
			{
				// r29: dual FOV circles - dim green = far cone, yellow = near cone
				int rn = ( g_CVars.Legit.FovNear * screen_y ) / 75;
				int rf = ( g_CVars.Legit.FovFar * screen_y ) / 75;
				if( rn < 3 ) rn = 3; if( rf < 3 ) rf = 3;
				if( g_CVars.Legit.FovSwitchDist > 0 )
				{
					g_Drawing.DrawCircle( screen_x / 2, screen_y / 2, rf, Color( 120, 255, 120, 90 ) );
					g_Drawing.DrawCircle( screen_x / 2, screen_y / 2, rn, Color( 255, 220, 80, 130 ) );
				}
				else
					g_Drawing.DrawCircle( screen_x / 2, screen_y / 2, rn, Color( 120, 255, 120, 110 ) );
			}
		}
