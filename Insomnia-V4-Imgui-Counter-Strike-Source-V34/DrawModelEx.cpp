...
int __stdcall Hooked_DrawModelEx( ModelRenderInfo_t &pInfo ) 
{
	...
	int chamStyle = g_CVars.Visuals.Chams.Style;
	if( chamStyle < 0 || chamStyle > 3 ) chamStyle = 0;
	IMaterial* in = ( chamStyle == 1 ) ? mat_vertex_in : mat_unlit_in;
	IMaterial* out = ( chamStyle == 1 ) ? mat_vertex_out : mat_unlit_out;
	in->SetMaterialVarFlag( MATERIAL_VAR_WIREFRAME, chamStyle == 2 );
	out->SetMaterialVarFlag( MATERIAL_VAR_WIREFRAME, chamStyle == 2 );
	in->SetMaterialVarFlag( MATERIAL_VAR_ADDITIVE, chamStyle == 3 );
	out->SetMaterialVarFlag( MATERIAL_VAR_ADDITIVE, chamStyle == 3 );
