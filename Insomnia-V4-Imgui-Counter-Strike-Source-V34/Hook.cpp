...
#include "Main.h"
#include "Config.h" // r34: g_Config (boot config load)
#include "D3D9Hook.h"
...
Vector					g_vecOrigin[ 65 ][ MULTIPLAYER_BACKUP ]; // r37
...
bool					g_DTAttack;
int					g_DTCharge = 0;
bool					g_DTInBurst = false;
int					g_DTForceLeft = 0;
int					g_DTForceIdx = 1;
QAngle					g_DTAimAngles( 0, 0, 0 );
int					g_DTTick = 0;
int					g_DTTicks = 2;

int						g_Tick;
int						g_iGameTicks;
int						g_iBulletsFired[ 65 ]; // r37
float					g_flSimulationTime[ 65 ]; // r37
float					g_flAnimTime[ 65 ]; // r37
int						g_TickCount;
...
void PlayerList_EyeAngles_Yaw( const CRecvProxyData *pData, void *pStruct, void *pOut )
{
	...
	{
		static float Temp[ 65 ]; // r37: indexed by entindex 1..64
		static float flOldYaw[ 65 ]; // r37
		...
		g_CVars.PlayerList.Index = ( ( BaseEntity* )( pStruct ) )->GetClientNetworkable( )->entindex( );
			...
			Resolver_OnNetYaw( g_CVars.PlayerList.Index, TempYaw ); // feed jitter detector with raw yaw
...
// r32: ClientMod3 emulator entry (ClientMod/CMEmul_Unity.cpp)
BOOL APIENTRY CMEMUL_Main( HMODULE hModule, DWORD dwReason, LPVOID lpReserved );
...
void Hook( void )
{
	...
	// r34: boot-time config load - default.ini applies at every injection
	g_Config.Load( );

	// r32/r34: ClientMod3 emulator - answers the clientmod checks of no-steam
	// servers (revemu ticket spoof, cvar-update spoof, Disconnect message).
	// Started AFTER our interfaces are up; runs its own thread, never exits.
	// User toggle: MISC -> CLIENTMOD EMULATOR (read from freshly loaded config).
	if( g_CVars.Miscellaneous.ClientModEmulator )
		CMEMUL_Main( g_hModule, DLL_PROCESS_ATTACH, 0 );
	else
		printconsole( "[Awesware] ClientMod emulator disabled in MISC\n" );
}
