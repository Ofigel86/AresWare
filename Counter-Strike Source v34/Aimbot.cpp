...
namespace Feature
{
	...
	/*
		...
		*/
	void Aimbot::OnCreateMove(CUserCmd* cmd, C_WeaponCSBaseGun* weapon)
	{
		...
		auto shooting = (cmd->buttons & IN_ATTACK && weapon->IsFireTime());

		if (!Config::Current->Aimbot->Mode) // Off
			return;

		if (Config::Current->Aimbot->Mode != 0 & Config::Current->Aimbot->Silent & Config::Current->Aimbot->AutoFire == 0)
		{
			if (!shooting)
			{
		...
		if (Config::Current->Aimbot->Mode == 1 & Config::Current->Aimbot->AutoFire)
		...
		{	
			...
			if (Config::Current->Aimbot->Height)
			{
				m_vTarget.z += (Config::Current->Aimbot->HeightScale);
			}
			m_vTarget.x += (Config::Current->Aimbot->HeightScaleX);
			m_vTarget.y += (Config::Current->Aimbot->HeightScaleY);
	...
	void Aimbot::ChangeTarget()
	{
		...
		for (int i = 1; i <= iMax; i++)
		{
			...
			if (IsTargetGood(pTarget))
			{
				...
				{
					int x, y;
					Source::m_pEngine->GetScreenSize(x, y);
	...
	bool Aimbot::IsTargetGood(C_CSPlayer* pTarget)
	{
		...
		auto& lc1 = LagCompensation1::Instance();
		auto& lc = LagCompensation::Instance();
		if (pTarget->m_lifeState() == LIFE_ALIVE && Config::Current->Aimbot->SetAbs)
		{
			pTarget->SetAbsAngles(pTarget->m_angEyeAngles());
			pTarget->SetAbsOrigin(pTarget->m_vecOrigin());
		}
		if (Config::Current->Aimbot->LagCompensation == 0)
		{
			if (!pTarget->GetHitboxVector(Config::Current->Aimbot->Spot, m_vTarget))
				return false;
		}

		if (Config::Current->Aimbot->LagCompensation == 1) // back
		{
			if (!lc.BacktrackPlayer(pTarget, m_pCmd, m_vTarget))
				if (!pTarget->GetHitboxVector(Config::Current->Aimbot->Spot, m_vTarget))
					return false;
		}
		if (Config::Current->Aimbot->LagCompensation == 2) // front
		{
			if (!lc1.BacktrackPlayer1(pTarget, m_pCmd, m_vTarget))
	//			if (!pTarget->GetHitboxVector(Config::Current->Aimbot->Spot, m_vTarget))
					return false;
		}
		if (Config::Current->Aimbot->LagCompensation == 3) // both
		{
			if (!lc1.BacktrackPlayer1(pTarget, m_pCmd, m_vTarget))
				if (!lc.BacktrackPlayer(pTarget, m_pCmd, m_vTarget))
					if (!pTarget->GetHitboxVector(Config::Current->Aimbot->Spot, m_vTarget))
	...
	for (int i = 1; i <= Source::m_pEngine->GetMaxClients(); i++)
	{
		...
		if (pTarget->GetIndex() == m_pLocal->GetIndex())
			return false;
		...
		if (m_pLocal->m_lifeState() != LIFE_ALIVE)
			...
			if (pTarget->m_MoveType() == MOVETYPE_NOCLIP)
				if (!lc.BacktrackPlayer(pTarget, m_pCmd, m_vTarget))
					return false;
			...
			if (Config::Current->Aimbot->AntiSpawnProtection)
			{
				if (pTarget->m_iHealth() > 110) // Health check	
					return false;
			}
			if (Config::Current->Aimbot->Target == 1) // Enemy
			{
