#if !defined( __VECTOR_H__ ) && !defined( VECTOR_H ) // r47: hl2sdk mathlib/vector.h wins in the dllmain/CMEmul TU
#define __VECTOR_H__
// r47: local float helpers (unaffected by the global min/max macros Valve headers install)
...
{
	result.x = vmin(a.x, b.x);
	result.y = vmin(a.y, b.y);
	result.z = vmin(a.z, b.z);
}

inline void VectorMax( const Vector &a, const Vector &b, Vector &result )
{
	result.x = vmax(a.x, b.x);
	result.y = vmax(a.y, b.y);
	result.z = vmax(a.z, b.z);
}
...
{
	return 1.f/vmax(1.f, v[0]*v[0]+v[1]*v[1]+v[2]*v[2]);
}
