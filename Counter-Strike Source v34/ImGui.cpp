...
("##On", &b);   // Label = "",       ID = hash of "##On" (no label!)
...
static void             LoadSettings(const char* filename);
...
ImGuiStyle::ImGuiStyle()
{
	...
	FramePadding            = ImVec2(8,2);      // Padding within a framed rectangle (used by most widgets)
	FrameRounding           = 0.0f;             // Radius of frame corners rounding. Set to 0.0f to have rectangular frames (used by most widgets).
	ItemSpacing             = ImVec2(9,2);      // Horizontal and vertical spacing between widgets/lines
	ItemInnerSpacing        = ImVec2(2,2);      // Horizontal and vertical spacing between within elements of a composed widget (e.g. a slider and its label)
	...
	Colors[ImGuiCol_WindowBg] = ImVec4(0.00f, 0.00f, 0.00f, 0.79f);
	...
	Colors[ImGuiCol_FrameBg] = ImVec4(0.00f, 1.00f, 0.00f, 0.39f);
	Colors[ImGuiCol_FrameBgHovered] = ImVec4(1.00f, 0.00f, 0.00f, 0.28f);
	Colors[ImGuiCol_FrameBgActive] = ImVec4(1.00f, 0.00f, 0.00f, 0.57f);
	Colors[ImGuiCol_TitleBg] = ImVec4(1.00f, 1.00f, 0.94f, 0.44f);
	Colors[ImGuiCol_TitleBgCollapsed] = ImVec4(0.96f, 0.40f, 0.80f, 0.20f);
	Colors[ImGuiCol_TitleBgActive] = ImVec4(1.00f, 0.32f, 0.63f, 1.00f);
	Colors[ImGuiCol_MenuBarBg] = ImVec4(0.00f, 0.00f, 0.00f, 1.00f);
	Colors[ImGuiCol_ScrollbarBg] = ImVec4(0.20f, 0.25f, 0.30f, 0.00f);
	Colors[ImGuiCol_ScrollbarGrab] = ImVec4(1.00f, 1.00f, 1.00f, 0.35f);
	Colors[ImGuiCol_ScrollbarGrabHovered] = ImVec4(1.00f, 1.00f, 1.00f, 0.64f);
	...
	Colors[ImGuiCol_Button] = ImVec4(0.67f, 1.00f, 1.00f, 0.40f);
...
ImGuiIO::ImGuiIO()
{
	...
	IniSavingRate = 5.0f;
	IniFilename = "ValveHookGui.ini";
	LogFilename = "ValveHookGuiLog.txt";
...
void ImGui::LoadSettings(const char* name)
{
	ImGuiContext& g = *GImGui;
	std::string current(name);
	if (current.find(XorStr(".theme")) == std::string::npos)
		current.append(XorStr(".theme"));
	...
	for (const char* line_start = file_data; line_start < buf_end; )
	{
		...
		else if (settings)
		{
			float x, y;
			int i;
			if (sscanf(line_start, "position=%f,%f", &x, &y) == 2)
...
void ImGui::SaveSettings(const char* name)
{
	ImGuiContext& g = *GImGui;
	std::string current(name);
	if (current.find(XorStr(".theme")) == std::string::npos)
...
void ImGui::Render()
{
	...
	if (g.Style.Alpha > 0.0f)
	{
		...
		if (g.IO.MouseDrawCursor)
		{
			...
			g.OverlayDrawList.AddImage(tex_id, pos, pos + size, cursor_data.TexUvMin[0], cursor_data.TexUvMax[0], IM_COL32(0, 255, 0, 255)); // White fill
			g.OverlayDrawList.PopTextureID();
		}
...
void ImGui::BeginTooltip()
{
	...
	ImGui::Begin(XorStr("##Tooltip"), NULL, flags);
}
...
void ImGui::OpenPopupEx(const char* str_id, bool reopen_existing)
{
	...
	ImGuiPopupRef popup_ref = ImGuiPopupRef(id, window, window->GetID(XorStr("##menus")), g.IO.MousePos); // Tagged as new ref because constructor sets Window to NULL (we are passing the ParentWindow info here)
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0 && !(flags & ImGuiWindowFlags_NoResize))
			{
				...
				const ImGuiID resize_id = window->GetID(XorStr("#RESIZE"));
				bool hovered, held;
...
bool MyIsKeyReleased(int key_index)
{
	ImGuiContext& g = *GImGui;
	if (key_index < 0) return false;
	IM_ASSERT(key_index >= 0 && key_index < IM_ARRAYSIZE(g.IO.KeysDown));
	...
	{
		if (!g.IO.MouseDown[key_index] && g.IO.MouseDownDuration[key_index] >= 0.0f)
			return true;
	}
	else
	{
...
bool ImGui::ColorPicker(float col[3])
{
	static const float HUE_PICKER_WIDTH = 20.0f;
	static const float CROSSHAIR_SIZE = 7.0f;
	static const ImVec2 SV_PICKER_SIZE = ImVec2(200, 200);
	...
	col[1] = color.Value.y;
	col[2] = color.Value.z;
	//RenderFrame(ImVec2(230, 30), ImVec2(230, 30), GetColorU32(col), false);
	return value_changed;
}
...
bool ImGui::Checkbox1(const char* label, bool* v)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	const bool user_scrolled = is_multiline && g.ActiveId == 0 && edit_state.Id == id && g.ActiveIdPreviousFrame == draw_window->GetID(XorStr("#SCROLLY"));
	...
	if (g.ActiveId == id || (edit_state.Id == id && is_multiline && g.ActiveId == draw_window->GetID(XorStr("#SCROLLY"))))
	{
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
	...
	bool menuset_is_open = !(window->Flags & ImGuiWindowFlags_Popup) && (g.OpenPopupStack.Size > g.CurrentPopupStack.Size && g.OpenPopupStack[g.CurrentPopupStack.Size].ParentMenuSet == window->GetID(XorStr("##menus")));
	if (menuset_is_open)
		g.FocusedWindow = window;
