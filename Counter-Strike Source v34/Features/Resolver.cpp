...
// ---------------------------------------------------------------------------
// Segregation animstate fix (their Compute_Torso_Rotation.hpp transplant):
// before the client recomputes body-yaw pose params for this frame, force
//   m_flGoalFeetYaw = m_flEyeYaw - offset
// on each resolved enemy's animation object, and trip the engine's torso
// rotation routine. The upper body (and therefore every head bone produced
// by SetupBones afterwards) then lands on the resolved yaw instead of the
// networked fake - without this the aimbox stays wherever the antiaim put it.
//
// No-steam client layout (same binary Segregation targeted; our own hook
// family already carries absolute engine addresses):
//   C_BasePlayer + 3968 -> animation state holder; object base = holder - 148
//   object + 24 -> m_flEyeYaw, object + 64 -> m_flGoalFeetYaw (ep1c SDK names)
// ---------------------------------------------------------------------------

static bool  g_TorsoSupported = true;				// flipped off on bad binary
static bool  g_TorsoAnnounced = false;

void Resolver_SetTorsoRotation( BasePlayer* ent, int idx )
{
	if( !g_TorsoSupported ) return;
	if( !g_CVars.Aimbot.Resolver.Active ) return;
	if( !ent || idx == 0 || idx > g_pGlobals->maxClients ) return;
	// local player: ROT - his animstate is faked by our own AA and the
	// engine must keep reflecting it
	if( idx == g_pEngineClient->GetLocalPlayer( ) ) return;

	ResolverPlayerState& st = g_ResolverState[ idx ];
	if( !st.initialized ) return;

	float offset = st.memoryActive ? st.memorizedOffset : st.appliedOffset;

	DWORD as_holder = *( DWORD* )( ( DWORD )ent + 3968 );
	if( as_holder < 0x10000 || as_holder < ( DWORD )ent ) return;

	DWORD aso = as_holder - 148;

	float* eyeYaw  = ( float* )( aso + 24 );
	float* goalFeetYaw = ( float* )( aso + 64 );

	// sanity: eye yaw must be a plausible angle; if the object layout shifted
	// (different client build), reading garbage here would be a float mess
	if( !( *eyeYaw > -400.f && *eyeYaw < 400.f ) ) { g_TorsoSupported = false; return; }

	*goalFeetYaw = *eyeYaw - offset;

	// their Set_Torso_Rotation(engine VA 604022992 = 0x2400A8D0), called with
	// remainder(eye - goal, 360) - triggers the body-yaw pose recompute
	typedef float( __thiscall* SetTorsoRotation_t )( void* animstate, float rotation );
	static SetTorsoRotation_t SetTorsoRotation = ( SetTorsoRotation_t )0x2400A8D0;

	if( !g_TorsoAnnounced )
	{
		unsigned char* fn = ( unsigned char* )SetTorsoRotation;
		bool sane = ( fn[ 0 ] == 0x55 || fn[ 0 ] == 0x56 || fn[ 0 ] == 0x57 ||
			fn[ 0 ] == 0x6A || fn[ 0 ] == 0xE9 || fn[ 0 ] == 0xEB );
		if( !sane )
		{
			g_TorsoSupported = false;
			g_TorsoAnnounced = true;
			Logger::Write( "[resolver] animstate fix unsupported on this binary build" );
			return;
		}
		g_TorsoAnnounced = true;
	}

	( float )SetTorsoRotation( ( void* )aso, Resolver_Norm( *eyeYaw - *goalFeetYaw ) );
}
