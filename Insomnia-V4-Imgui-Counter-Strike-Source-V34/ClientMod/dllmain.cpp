// BUILD MARKER r32 (2026-09-18): vendored ClientMod3 Emulator (Informant/0TheSpy) - anti-detect for clientmod-checked no-steam servers.
...
#include <iostream>  
#include <random>
#include <ctime> // r47: mingw needs explicit ctime for std::time/tm/localtime/strftime
#include <string>
// r47: Windows.h moved BELOW the C++ std headers - its min/max macros break libstdc++
// (fstream.tcc) when preprocessed first; harmless for MSVC either way.
#include <Windows.h>
...
// r47: std::ofstream -> FILE* (libstdc++ instantiates basic_filebuf lazily, AFTER the
// min/max macros from windows.h/valve headers exist -> parse error in fstream.tcc under mingw)
FILE* logfile = NULL;
#ifdef DEBUG
...
void printfdbg(const char* format, ...)
{
	...
	char timebuffer[ 32 ];
	strftime( timebuffer, sizeof( timebuffer ), "[%H:%M:%S] ", std::localtime( &time ) ); // r47: was cout/put_time
	printf( "%s", timebuffer );
	if( logfile ) fprintf( logfile, "%s", timebuffer );
	...
	va_end(arglist);
	printf(logstring);
	if( logfile ) fprintf( logfile, "%s", logstring ); // r47: was logfile <<
}
#else
#define printfdbg(...)
#endif
...
IVEngineClient* cm_pEngineClient = 0;
...
bool __fastcall Hooked_PrepareSteamConnectResponse(DWORD* ecx, void* edx, int keySize, const char* encryptionKey, uint64 unGSSteamID, bool bGSSecure, const netadr_t& adr, bf_write& msg)
{
	...
	if (!RevSpoofer::Spoof(hwid, steamid)) {
		...
		CallVFunction<IVEngineClient* (__thiscall*)(void*, char*)>(cm_pEngineClient, 97)(cm_pEngineClient, "retry");
		return false;
	}
...
CGameEventManager* cm_GameEventManager;
...
const char* GetEventName(int eventid)
{
	...
	// r47: plain C reads instead of the MSVC asm block (same offsets, both compilers)
	count = *( int* )( ( DWORD )cm_GameEventManager + 0x10 );
	descriptors = *( CGameEventDescriptor** )( ( DWORD )cm_GameEventManager + 0x4 );
	for (size_t i = 0; i < count; i++)
	{
...
bool __fastcall hkSendNetMsg(INetChannel* this_, void* edx, INetMessage& msg, bool bVoice)
{
	...
	if (!srcds) {
		if (cmd == net_SignonState)
		{
			...
			{
				CallVFunction<IVEngineClient* (__thiscall*)(void*, char*)>(cm_pEngineClient, 97)(cm_pEngineClient,
...
{
	// r47: was "__asm mov ClassID, edx"; same edx capture for mingw
	{ int c; __asm__ __volatile__ ( "movl %%edx, %0" : "=r" (c) ); ClassID = c; }
	...
	if (V_stricmp(this_->GetName(), "cm_log") == 0)
	{
		...
		if (newlog != -1 && newlog != g_pCVar->FindVar("cm_log")->GetInt())
		{
			...
			if (newlog == 1)
			{
				...
				logfile = fopen( logname, "a" ); // r47: was ofstream.open
				if (!logfile) {
					printfdbg("Failed to open\n"); // r47: was cout
				}
			}
			...
			if (newlog == 0)
			{
				//saveLogFile ;
				printfdbg("Saved log\n");
				if( logfile ) { fclose( logfile ); logfile = NULL; } // r47: was ofstream.close
			}
		}
	}
...
void ConsoleInputThread(HMODULE hModule)
{
	...
	while (true) {
		...
		CallVFunction<IVEngineClient* (__thiscall*)(void*, char*)>(cm_pEngineClient, 97)(cm_pEngineClient, //cm_pEngineClient->ExecuteClientCmd
			input);
	}
}
...
DWORD WINAPI HackThread(HMODULE hModule)
{
...
#else
	AllocConsole(); FILE* f; freopen_s(&f, "CONOUT$", "w", stdout); ShowWindow(GetConsoleWindow(), SW_HIDE);
#endif
	...
	cm_GameEventManager = (CGameEventManager*)GetInterface("engine.dll", "GAMEEVENTSMANAGER002");

	cm_pEngineClient = (IVEngineClient*)GetInterface("engine.dll", "VEngineClient012");
	...
	while (true)
	{
		if (false) // r32: uninject hotkey disabled
			break;
	...
	return 0; // r32: never unload the host cheat module

	return 0;
}

#ifdef CMEMUL_STATIC
BOOL APIENTRY CMEMUL_Main(HMODULE hModule,
	DWORD  dwReason,
	LPVOID lpReserved
)
#else
